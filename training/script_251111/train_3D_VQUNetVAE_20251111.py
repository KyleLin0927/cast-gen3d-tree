#!/usr/bin/env python3
"""
Minecraft 3D U-Net VQ-VAE (32x32x32 voxels, 3 classes: 0=air,1=oak_log,2=oak_leaves)

核心設計：

- Encoder:
    32^3 -> 16^3 -> 8^3 -> 4^3
    最終輸出連續 latent z_e: [B, latent_dim, 4,4,4]

- Vector Quantizer (VQ-VAE):
    將 z_e 量化成離散 code:
        z_q = e_k (from codebook)
    使用標準 VQ loss:
        L_vq = ||sg[z_e] - e||^2 + beta * ||z_e - sg[e]||^2
    並使用 straight-through estimator 反傳。

- Decoder:
    4^3 -> 8^3 -> 16^3 -> 32^3
    預設支援 skip_levels (U-Net style)，但強烈建議
    VQ 訓練時關閉 skip (no_skip_connections 或 skip_levels=0)，
    讓 latent 真正承擔結構資訊，方便後續 Transformer prior。

- Loss:
    CE(recon_logits, labels, class_weights) + vq_loss_weight * L_vq
...
Output:
  與原本類似：
    ...
    - metadata CSV
    - samples/ 下保存重建結果 (npz + 3-view PNG)
"""
...
class VectorQuantizer(nn.Module):
    """
    標準 VQ-VAE codebook (non-EMA).
    z_e: [B, C, D, H, W]
    回傳:
      z_q: quantized latents (with straight-through)
      loss: VQ loss (包含 commitment cost)
      perplexity: 使用 code 的熵指標
      encodings_idx: [B, D, H, W] code indices
    """
    ...
    def forward(self, z_e: torch.Tensor):
        # z_e: [B, C, D, H, W]
        B, C, D, H, W = z_e.shape
        flat = z_e.permute(0, 2, 3, 4, 1).contiguous().view(-1, C)  # [N, C], N=B*D*H*W
        ...
        # 取最近的 code index
        encodings_idx = torch.argmin(dist, dim=1)  # [N]
        encodings_onehot = F.one_hot(encodings_idx, self.num_embeddings).type(flat.dtype)

        # 對應的量化向量
        z_q = encodings_onehot @ emb  # [N, C]
        z_q = z_q.view(B, D, H, W, C).permute(0, 4, 1, 2, 3).contiguous()  # [B,C,D,H,W]

        # VQ 損失
        # e_latent_loss: codebook 和 encoder output 對齊
        # q_latent_loss: commitment，避免 encoder 瘋狂跳
...
class Encoder3DVQVAE(nn.Module):
    ...
    Down: 32 -> 16 -> 8 -> 4
    輸出:
      z_e: [B, latent_dim, 4,4,4]
      skips: (e1,e2,e3) for optional U-Net skip connections.
    """
    ...
    def forward(self, x, return_skips: bool = True):
        e1 = self.enc1(x)          # [B,base,32,32,32]
        e2 = self.enc2(e1)         # [B,base*2,16,16,16]
        e3 = self.enc3(e2)         # [B,base*4,8,8,8]
        e4 = self.enc4(e3)         # [B,base*8,4,4,4]
        z_e = self.to_latent(e4)   # [B,latent_dim,4,4,4]
...
class Decoder3DVQVAE(nn.Module):
    """
    Up: 4 -> 8 -> 16 -> 32
    若 skip_levels>0 則使用 encoder skips 做 U-Net concat。
    """
    ...
    def forward(self, z_q, skips=None):
        ...
        h = self.up1(z_q)   # [B,base*8,8,8,8]
        h = self.rb1(h)

        if self.skip_levels >= 1:
            h = torch.cat([h, e3], dim=1)
        h = self.up2(h)     # [B,base*4,16,16,16]
        h = self.rb2(h)

        if self.skip_levels >= 2:
            h = torch.cat([h, e2], dim=1)
        h = self.up3(h)     # [B,base*2,32,32,32]
        h = self.rb3(h)
...
def train(args, resume_checkpoint=None):
    ...
    # Header
    header_text = (
        f"[bold cyan]Minecraft 3D U-Net VQ-VAE Training[/bold cyan]\n"
    ...
    if skip_levels_effective == 0:
        ...
        if skip_levels_effective < 3:
            skip_msg += " (from deepest only)"
    console.print(f"[bold]Skip connections (Decoder):[/bold] {skip_msg}")
    if skip_levels_effective > 0:
        console.print("[yellow]Note:[/yellow] For VQ-VAE + prior，建議實驗時先關掉 skip 讓 latent 扛資訊。")
    ...
    model = UNet3DVQVAE(
        ...
        num_embeddings=args.vq_num_codes,
    ...
    # Resume (如果你想加也可沿用原本邏輯，這裡簡化略過 resume_checkpoint 合併 args)
    if resume_checkpoint:
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            for batch_idx, (onehot, labels) in enumerate(train_loader):
                ...
                with torch.amp.autocast(device_type=device.type, enabled=use_amp):
                    logits, vq_loss, perplexity = model(onehot)
                    ...
                    if epoch == start_epoch and batch_idx == 0 and not logits_stats_collected:
                        ...
                        console.print(
                            f"[dim]VQ: loss={vq_loss.item():.4f}, perplexity={perplexity.item():.2f}[/dim]"
                        )
                    ...
                    total_loss = ce + args.vq_loss_weight * vq_loss
                ...
                train_total_sum += total_loss.detach().item() * batch_size
                train_ce_sum += ce.detach().item() * batch_size
                train_vq_sum += vq_loss.detach().item() * batch_size
                progress.update(epoch_task, advance=1)

                if epoch == start_epoch and batch_idx == 0:
                    console.print(
                        f"Debug: CE={ce.item():.4f}, "
                        f"VQ={vq_loss.item():.4f} (x{args.vq_loss_weight}) → total={total_loss.item():.4f}"
                    )

            train_loss = train_total_sum / max(train_sample_count, 1)
            train_ce_avg = train_ce_sum / max(train_sample_count, 1)
            train_vq_avg = train_vq_sum / max(train_sample_count, 1)
            ...
            val_ce_avg = float('inf')
            val_vq_avg = float('inf')
            ...
            if interrupted['flag']:
                ...
                with torch.no_grad():
                    for onehot, labels in val_loader:
                        ...
                        loss = ce + args.vq_loss_weight * vq_loss
                        batch_size = labels.size(0)
                        val_sample_count += batch_size
                        val_total_sum += loss.item() * batch_size
                        val_ce_sum += ce.item() * batch_size
                        val_vq_sum += vq_loss.item() * batch_size
                        progress.update(epoch_task, advance=1)
                val_loss = val_total_sum / max(val_sample_count, 1)
                val_ce_avg = val_ce_sum / max(val_sample_count, 1)
                val_vq_avg = val_vq_sum / max(val_sample_count, 1)
                progress.remove_task(epoch_task)
            ...
            if interrupted['flag']:
                ...
                save_checkpoint(save_epoch, is_last=True)
            ...
            training_history.append({
                ...
                'train_vq': train_vq_avg,
                'val_loss': val_loss,
                'val_ce': val_ce_avg if not interrupted['flag'] else float('inf'),
                'val_vq': val_vq_avg if not interrupted['flag'] else float('inf'),
            ...
            val_cr_text = f"{val_ce_avg:.6f}" if not math.isinf(val_ce_avg) else "inf"
            val_kl_text = f"{val_vq_avg:.6f}" if not math.isinf(val_vq_avg) else "inf"
            progress.console.print(
                f"Epoch {epoch:03d}: train {train_loss:.6f} (CR {train_ce_avg:.6f}, KL {train_vq_avg:.6f}) | "
                f"val {val_loss:.6f} (CR {val_cr_text}, KL {val_kl_text}) | {fmt_secs(epoch_secs)}"
            ...
            if epoch % args.sample_every == 0:
                ...
                with torch.no_grad():
                    for onehot, labels in val_loader:
                        ...
                        # Posterior samples: 使用 encoder+VQ 的量化結果重新 decode
                        # (目前還沒有 prior，所以只是 sanity check)
                        for i in range(min(args.n_samples, logits.shape[0])):
    ...
    final_table.add_row(
        ...
        else (last_checkpoint_path if os.path.exists(last_checkpoint_path) else "None")
    )
    console.print("\n", final_table, "\n")
    ...
    with open(csv1_path, 'w', newline='') as f:
        fieldnames = ['epoch', 'train_loss', 'train_ce', 'train_vq',
                      'val_loss', 'val_ce', 'val_vq',
    ...
    ce_desc = (
        ...
        if class_weights_base is not None else "CrossEntropyLoss(weight=None)"
    )
    loss_formula = (
        f"Loss = {ce_desc} + {args.vq_loss_weight} * VQ_Loss"
    )
    ...
    metadata = {
        ...
        'last_checkpoint_path': 'deleted_after_completion' if last_checkpoint_removed
                               else (last_checkpoint_path if os.path.exists(last_checkpoint_path) else 'not_available'),
        ...
        'loss_vq': '||sg[z_e]-e||^2 + beta * ||z_e-sg[e]||^2',
    }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Minecraft Tree 3D U-Net VQ-VAE")
    ...
    parser.add_argument('--latent_dim', type=int, default=256,
                        help='Channels of spatial latent at 4x4x4 (embedding_dim)')

    parser.add_argument('--skip_levels', type=int, default=0,
                        choices=[0, 1, 2, 3],
                        help='Decoder skip connection levels (0 disables skips)')

    # VQ-VAE 超參
    parser.add_argument('--vq_num_codes', type=int, default=512,
                        help='Number of codebook embeddings')
    parser.add_argument('--vq_beta', type=float, default=0.25,
                        help='VQ commitment beta')
    parser.add_argument('--vq_loss_weight', type=float, default=1.0,
                        help='Global weight for VQ loss in total loss')
    ...
    parser.add_argument('--no_skip_connections', action='store_true',
                        help='Disable all decoder skip connections (overrides --skip_levels)')
    ...
    if not args.out_dir and not args.resume:
        parser.error("--out_dir is required (unless resuming with it set).")
