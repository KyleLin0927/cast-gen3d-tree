...
# ---------------------------------------------------------------------------
# VQ-VAE variant (for compatibility with VQ checkpoints)
# ---------------------------------------------------------------------------
...
def eval_model(
    ...
    if skip_levels is None:
        skip_levels = 3 if not is_vq else 0
    ...
    if is_vq:
        model_params.setdefault("additional_notes", [])
        note = f"detected_vq(num_embeddings={num_embeddings}, beta={beta}, skip_levels={skip_levels})"
        model_params["additional_notes"].append(note)
        compat_adjustments = list(compat_adjustments) if compat_adjustments else []
        compat_adjustments.append(note)
