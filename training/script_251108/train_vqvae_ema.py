...
class VectorQuantizer(nn.Module):
    """
    EMA-based VQ-VAE codebook (Exponential Moving Average).

    Uses exponential moving average to update codebook embeddings instead of gradients.
    This typically provides more stable training and better codebook utilization.
    ...
    def __init__(
        ...
        # Initialize embeddings (using buffer so they're not trainable parameters)
        # Force FP32 for numerical stability in EMA updates
        self.register_buffer('embedding', torch.randn(num_embeddings, embedding_dim, dtype=torch.float32))
        self.embedding.data.normal_()

        # EMA statistics: cluster size and embedding average (all FP32 for stability)
    ...
    def forward(self, z: torch.Tensor):
        ...
        # Get quantized embedding (FP32), then convert to input dtype for decoder AMP compatibility
        z_q = self.embedding[encoding_indices].view(B, D, H, W, C)
        z_q = z_q.permute(0, 4, 1, 2, 3).contiguous()  # [B, C, D, H, W]
        # Convert to input dtype to maintain AMP compatibility for decoder
        z_q = z_q.to(dtype=z.dtype)

        # Update EMA statistics during training
        if self.training:
            with torch.no_grad():
                # Count occurrences of each embedding (optimized: use bincount instead of one-hot)
                # Convert to FP32 for EMA stability
                cluster_size = torch.bincount(encoding_indices, minlength=self.num_embeddings).float()  # [K]

                # Update cluster size EMA
                self.cluster_size.mul_(self.decay).add_(cluster_size, alpha=1 - self.decay)

                # Compute embedding sum: sum of all z_e that map to each code (optimized: use index_add instead of matrix multiply)
                # Use FP32 for EMA stability (convert flat_z to FP32 for accumulation)
                embedding_sum = torch.zeros(self.num_embeddings, self.embedding_dim, device=z.device, dtype=torch.float32)  # [K, C]
                flat_z_fp32 = flat_z.float()  # Convert to FP32 for stable EMA accumulation
                embedding_sum.index_add_(0, encoding_indices, flat_z_fp32)  # [K, C]

                # Update embedding average EMA
                self.embedding_avg.mul_(self.decay).add_(embedding_sum, alpha=1 - self.decay)
        ...
        # Loss: only commitment loss (codebook is updated via EMA, not gradients)
        ...
        # Perplexity (optimized: use bincount instead of one-hot)
        counts = torch.bincount(encoding_indices, minlength=self.num_embeddings).float()  # [K]
        avg_probs = counts / encoding_indices.numel()  # [K]
...
class VQVAE3D(nn.Module):
    ...
    def decode_from_indices(self, indices: torch.Tensor):
        ...
        # In EMA version, embedding is a buffer, so we index it directly
...
def train(args, resume_checkpoint=None):
    ...
    console.print(f"[bold]VQ-VAE (EMA):[/bold] codebook_size={args.codebook_size}, latent_dim={args.latent_dim}, vq_beta={args.vq_beta}, ema_decay={args.ema_decay}, ema_epsilon={args.ema_epsilon}\n")
    ...
    loss_formula = (
        f"Loss = {ce_desc} + {args.vq_beta} * VQ_Loss(commitment only, codebook updated via EMA)"
    )
    current_script = (
    ...
    initial_metadata = {
        ...
        "loss_vq": f"VQ_Loss(commitment only: {0.25} * commitment, codebook updated via EMA with decay={args.ema_decay})",
    ...
    emb = model.vq.embedding.detach().cpu().numpy()  # [K, C] (EMA version: embedding is a buffer, not nn.Embedding)
    K, C = emb.shape
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.99,
        help="EMA decay rate for codebook updates (default: 0.99)",
    )
    parser.add_argument(
        "--ema_epsilon",
        type=float,
        default=1e-5,
        help="EMA epsilon for numerical stability (default: 1e-5)",
    )
