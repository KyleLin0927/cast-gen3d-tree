#!/usr/bin/env python3
"""
Minecraft 3D U-Net VAE + Optional Teacher Distillation

- Student: UNet3DVAE
- (Optional) Teacher:
    * 指定 --teacher_ckpt 讀取已訓練好的 UNet3DVAE checkpoint
    * Teacher 參數完全凍結
    * Loss = CE(student, gt) [除非 --no_distill_ce] + kl_beta * KL(student)
             + distill_alpha_out * KD(student_logits || teacher_logits)

KD:
    - 對 channel 做 softmax / KL
    - 對 batch + 空間平均，避免因 voxel 數暴增
    - 正常情況下量級 ~ CE 同級，方便用直觀的 distill_alpha_out
"""
...
def extract_zip_to_temp(zip_path: str, console=None) -> tuple[str, tempfile.TemporaryDirectory]:
    ...
    with zipfile.ZipFile(zip_path, 'r') as zf:
        ...
        if console:
            console.print(f"[green]✓[/green] Extracted {len(zf.namelist())} items from zip")
    ...
    if not os.path.exists(train_dir):
        ...
        if len(found) >= 3:
            common_parent = os.path.commonpath(found)
    ...
    for d, name in [(train_dir, 'train'), (val_dir, 'val'), (test_dir, 'test')]:
        if not os.path.exists(d):
            raise ValueError(f"{name}/ not found (checked {d})")
...
class VoxelDataset(Dataset):
    """
    Loads 32x32x32 int labels from npz.
    Returns:
        onehot: [3,32,32,32]
        labels: [32,32,32]
    """
...
class Decoder3DUNetVAE(nn.Module):
    """
    4 -> 8 -> 16 -> 32 with configurable skip levels.
    """
    ...
    def forward(self, z, skips=None):
        if self.skip_levels > 0:
            if skips is None:
                raise ValueError("Decoder requires skips when skip_levels > 0")
            e1, e2, e3 = skips
        else:
            e1 = e2 = e3 = None
...
class UNet3DVAE(nn.Module):
    def __init__(self, in_ch=3, out_ch=3, base=64, latent_dim=256, skip_levels: int = 3):
        ...
        self.decoder = Decoder3DUNetVAE(out_ch, base, latent_dim, skip_levels=self.skip_levels)
    ...
    def forward(self, x):
        ...
        logits = self.decoder(z, skips if self.use_skip_connections else None)
...
def kl_divergence(mu, logvar):
    """
    Spatial KL(q(z|x) || N(0,I)) averaged over batch & all spatial dims.
    """
...
def kd_kl_loss(student_logits, teacher_logits, T: float = 1.0):
    """
    Per-voxel KD loss:
      - KL( p_t || p_s ) over class/channel dimension
      - 平均 over batch & 空間 (D,H,W)
      - 再乘上 T^2 (標準 KD scaling)

    這樣數值不會隨 32^3 體素暴漲，量級通常接近 CE，
    distill_alpha_out 就可以用 0.x ~ 1 的直觀範圍。
    """
    if T <= 0:
    ...
    # KL(p_t || p_s) over C
    # 避免 log(0)，加小 epsilon
    kl = (p_t * (torch.log(p_t + 1e-8) - log_p_s)).sum(dim=1)  # [B, D, H, W]
...
def build_teacher(args, device, console):
    if not args.teacher_ckpt:
        return None

    if not os.path.exists(args.teacher_ckpt):
        console.print(f"[red]Teacher checkpoint not found:[/red] {args.teacher_ckpt}")
        raise SystemExit(1)

    console.print(f"[cyan]Loading teacher from[/cyan] [magenta]{args.teacher_ckpt}[/magenta]")

    ckpt = torch.load(args.teacher_ckpt, map_location='cpu', weights_only=False)
    t_args = ckpt.get('args', {})

    t_base = t_args.get('base', args.base)
    t_latent = t_args.get('latent_dim', args.latent_dim)
    t_skip = t_args.get('skip_levels', 3)
    t_in_ch = 3
    t_out_ch = 3
    ...
    console.print(f"[green]✓[/green] Teacher loaded "
                  f"(base={t_base}, latent_dim={t_latent}, skip_levels={t_skip}) and frozen.")
    return teacher
...
def train(args, resume_checkpoint=None):
    ...
    skip_levels_effective = 0 if args.no_skip_connections else args.skip_levels
    args.skip_levels_effective = skip_levels_effective
    if args.no_skip_connections and args.skip_levels > 0:
    ...
    # Header
    header_text = (
        f"[bold cyan]Minecraft 3D U-Net VAE Training (Student + optional Teacher KD)[/bold cyan]\n"
    ...
    if class_weights_base is None:
        ...
        console.print(f"[dim]  air={class_weights_base[0]:.4f}, "
    ...
    if skip_levels_effective == 0:
        skip_msg = "DISABLED (levels=0)"
    else:
        skip_msg = f"ENABLED (levels={skip_levels_effective})"
        if skip_levels_effective < 3:
            skip_msg += " starting from deepest"
    console.print(f"[bold]Skip connections (STUDENT):[/bold] {skip_msg}\n")
    ...
    # Teacher (optional)
    teacher = build_teacher(args, device, console)
    ...
    if device.type == 'cuda':
        console.print(f"[bold]AMP:[/bold] {'[green]ENABLED[/green]' if use_amp else '[yellow]DISABLED[/yellow]'}")
    else:
        console.print(f"[bold]AMP:[/bold] [dim]N/A[/dim]")

    if teacher is not None:
        console.print(f"[bold]Distillation:[/bold] "
                      f"alpha_out={args.distill_alpha_out}, "
                      f"T={args.distill_temperature}, "
                      f"CE_gt={'ON' if not args.no_distill_ce else 'OFF'}")
    else:
        console.print(f"[bold]Distillation:[/bold] [dim]No teacher (pure VAE)[/dim]")
    ...
    if resume_checkpoint:
        ...
        console.print(f"[green]✓[/green] Resumed from epoch {resume_checkpoint['epoch']}, "
    ...
    with Progress(
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            for batch_idx, (onehot, labels) in enumerate(train_loader):
                ...
                with torch.amp.autocast(device_type=device.type, enabled=use_amp):
                    ...
                    # CE with GT (unless disabled)
                    if args.no_distill_ce:
                        ce_loss = torch.tensor(0.0, device=device, dtype=logits.dtype)
                    else:
                    ...
                    total_loss = ce_loss + args.kl_beta * kl

                    # KD from teacher
                    if teacher is not None:
                        with torch.no_grad():
                            t_logits, _, _ = teacher(onehot)
                        kd = kd_kl_loss(logits, t_logits, T=args.distill_temperature)
                        total_loss = total_loss + args.distill_alpha_out * kd
                    else:
                        kd = torch.tensor(0.0, device=device, dtype=logits.dtype)
                ...
                total_loss_item = total_loss.detach().item()
                train_total_sum += total_loss_item * batch_sz
                train_ce_sum += ce_loss.detach().item() * batch_sz
                train_kl_sum += kl.detach().item() * batch_sz
                train_kd_sum += kd.detach().item() * batch_sz
                progress.update(epoch_task, advance=1)

                if epoch == start_epoch and batch_idx == 0:
                    console.print(
                        f"Debug: CE={ce_loss.detach().item():.4f}, "
                        f"KL={kl.detach().item():.4f}, "
                        f"KD={kd.detach().item():.4f}, "
                        f"total={total_loss.detach().item():.4f}"
                    )

            divisor = len(train_loader.dataset)
            train_loss = train_total_sum / divisor
            train_cr = train_ce_sum / divisor
            train_kl = train_kl_sum / divisor
            train_kd = train_kd_sum / divisor

            if epoch == start_epoch:
                total_voxels = class_counts.sum().item()
                if total_voxels > 0:
                    console.print(f"[dim]Class distribution (train, first epoch):[/dim]")
                    console.print(f"[dim]  air:   {class_counts[0].item():,} "
                                  f"({100*class_counts[0].item()/total_voxels:.1f}%)[/dim]")
                    console.print(f"[dim]  log:   {class_counts[1].item():,} "
                                  f"({100*class_counts[1].item()/total_voxels:.1f}%)[/dim]")
                    console.print(f"[dim]  leaves:{class_counts[2].item():,} "
            ...
            if interrupted['flag']:
                ...
                with torch.no_grad():
                    for onehot, labels in val_loader:
                        ...
                        kl = kl_divergence(mu, logvar)
                        loss = ce_loss + args.kl_beta * kl

                        if teacher is not None:
                            t_logits, _, _ = teacher(onehot)
                            kd = kd_kl_loss(logits, t_logits, T=args.distill_temperature)
                            loss = loss + args.distill_alpha_out * kd
                        else:
                            kd = torch.tensor(0.0, device=device, dtype=logits.dtype)

                        batch_sz = labels.size(0)
                        val_total_sum += loss.detach().item() * batch_sz
                        val_ce_sum += ce_loss.detach().item() * batch_sz
                        val_kl_sum += kl.detach().item() * batch_sz
                        val_kd_sum += kd.detach().item() * batch_sz
                        progress.update(epoch_task, advance=1)
                divisor_val = len(val_loader.dataset)
                val_loss = val_total_sum / divisor_val
                val_cr = val_ce_sum / divisor_val
                val_kl = val_kl_sum / divisor_val
                val_kd = val_kd_sum / divisor_val
                progress.remove_task(epoch_task)
            ...
            save_last = (epoch % args.save_every == 0) or (epoch == args.epochs)
            if save_last:
                save_checkpoint(epoch, is_last=True)

            best_marker = " | ★ Best!" if is_best else ""
            ckpt_marker = " | 💾 Saved" if save_last else ""
            progress.console.print(
                f"Epoch {epoch:03d}: "
                f"train {train_loss:.6f} [CR {train_cr:.6f} | KL {train_kl:.6f} | KD {train_kd:.6f}] | "
                f"val {val_loss:.6f} [CR {val_cr:.6f} | KL {val_kl:.6f} | KD {val_kd:.6f}] | "
                f"{fmt_secs(epoch_secs)}{best_marker}{ckpt_marker}"
            )

            # ------------- Samples -------------
            if epoch % args.sample_every == 0:
                sample_task = progress.add_task(
                    "[blue]Generating samples",
                    total=args.n_samples + 1
                )
                model.eval()
                with torch.no_grad():
                    for onehot, labels in val_loader:
                        ...
                        mu_b, logvar_b, skips = model.encoder(
                            onehot,
                            return_skips=model.use_skip_connections
                        )
        ...
        if os.path.exists(best_checkpoint_path):
            best_cp = torch.load(best_checkpoint_path, map_location=device, weights_only=False)
        ...
        with torch.no_grad():
            for onehot, labels in test_loader:
                ...
                running += loss.detach().item() * labels.size(0)
    ...
    final_table.add_row("Best Val Loss", f"{best_val:.9f}")
    final_table.add_row("Final Test Loss", f"{test_loss:.9f}")
    ...
    final_table.add_row("Best Model", best_checkpoint_path if os.path.exists(best_checkpoint_path) else "None")
    final_table.add_row(
        "Last Checkpoint",
        "Deleted after completion" if last_removed
    ...
    loss_formula = (
        f"Loss = "
        f"{'0' if args.no_distill_ce else ce_desc} + "
        f"{args.kl_beta} * KL + "
        f"{args.distill_alpha_out} * KD(student||teacher)"
    ...
    metadata = {
        ...
        'teacher_ckpt': args.teacher_ckpt if args.teacher_ckpt else 'None',
        'distill_alpha_out': args.distill_alpha_out,
        'distill_temperature': args.distill_temperature,
        'use_distill_ce_gt': bool_to_str(not args.no_distill_ce),
        'start_epoch': start_epoch,
        'end_epoch': args.epochs,
        'training_start_time': train_start_time.strftime('%Y-%m-%d %H:%M:%S'),
        'training_end_time': train_end_time.strftime('%Y-%m-%d %H:%M:%S'),
        'best_model_path': best_checkpoint_path if os.path.exists(best_checkpoint_path) else 'None',
        'last_checkpoint_path': (
            'deleted_after_completion' if last_removed
            else last_checkpoint_path if os.path.exists(last_checkpoint_path)
            else 'not_available'
        ),
        ...
        'loss_reconstruction': ce_desc if not args.no_distill_ce else '0',
        'loss_kl_divergence': 'KL(q(z|x) || N(0,I)) (spatial)',
        'loss_kd': 'KD(student_logits || teacher_logits)' if teacher is not None else 'None',
    }
    ...
    with open(csv3_path, 'w', newline='') as f:
        w = csv.DictWriter(f, fieldnames=metadata.keys())
        w.writeheader()
        w.writerow(metadata)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Minecraft Tree 3D U-Net VAE + Teacher Distillation"
    )
    ...
    parser.add_argument('--skip_levels', type=int, default=3, choices=[0, 1, 2, 3])
    ...
    parser.add_argument('--distill_alpha_out', type=float, default=0.7,
                        help='Weight for KD loss on logits')
    parser.add_argument('--distill_temperature', type=float, default=2.0,
                        help='Temperature for KD softmax')
    parser.add_argument('--no_distill_ce', action='store_true',
                        help='Disable CE with ground-truth (only distill from teacher)')
    ...
    if not args.data_root and not args.data_zip and not args.resume:
        raise SystemExit("Either --data_root or --data_zip is required (unless resuming).")
    if args.data_root and args.data_zip:
        raise SystemExit("Use only one of --data_root or --data_zip.")
    if not args.out_dir:
        raise SystemExit("--out_dir is required (unless resuming with it set).")
    if not args.exp_name:
        raise SystemExit("--exp_name is required (unless resuming with it set).")

    temp_dir_holder = []
    if args.data_zip and not args.data_root:
        console = Console()
