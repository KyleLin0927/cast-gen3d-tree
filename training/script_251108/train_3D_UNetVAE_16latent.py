...
This version uses a SHALLOW spatial bottleneck:

  - Encoder produces spatial latent map: mu, logvar shape [B, latent_dim, 16,16,16]
  - Only ONE downsampling step: 32 -> 16
  - Decoder upsamples: 16 -> 32
...
# ----------------------
# Model: 3D U-Net VAE (shallow 16x16x16 bottleneck)
# ----------------------
...
class Encoder3DUNetVAE(nn.Module):
    ...
    Returns:
      mu, logvar: [B, latent_dim, 16,16,16]
      skips: (e1, e2)
        e1: [B, base,   32,32,32]
...
class Decoder3DUNetVAE(nn.Module):
    """
    Up: 16 -> 32 with configurable skip connections.

    Input:
      z: [B, latent_dim, 16,16,16]
      skips: (e1,e2) from encoder when skip connections are enabled.

    skip_levels:
      0: no skip connections
      1: concat e2 at 16^3 before upsample
      2: concat e2 at 16^3 and e1 at 32^3
    """
        ...
        if self.use_skip_connections:
            if skips is None:
                ...
                if self.skip_levels >= 1:
                    # e2 channels = up.in_channels - latent_dim (from z)
                    e2_channels = self.up.in_channels - z.shape[1]
                    e2 = torch.zeros(B, e2_channels, 16, 16, 16, device=z.device, dtype=z.dtype)
                else:
                    e2 = None
                if self.skip_levels >= 2:
                    # e1 channels = out_block.in_channels - base*2 (from rb_up output)
                    base_2_channels = self.rb_up.net[0].weight.shape[1]  # base*2
                ...
                if len(skips) != 2:
                    raise ValueError("Expected 2 skip tensors (e1,e2) from encoder.")
                e1, e2 = skips
        else:
            e1 = e2 = None
...
class UNet3DVAE(nn.Module):
    ...
    def forward(self, x):
        ...
        z = self.reparameterize(mu, logvar)  # [B, latent_dim, 16,16,16]
...
def train(args, resume_checkpoint=None):
    ...
    if args.skip_levels < 0 or args.skip_levels > 2:
        console.print(
            Panel.fit(
                f"[bold red]ERROR: --skip_levels must be between 0 and 2 (got {args.skip_levels})[/bold red]",
                border_style="red",
            )
        )
        raise SystemExit(1)
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            if epoch % args.sample_every == 0:
                ...
                with torch.no_grad():
                    ...
                    latent_shape = (1, args.latent_dim, 16, 16, 16)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=256,
        help="Channels of spatial latent at 16x16x16",
    )
    ...
    parser.add_argument(
        ...
        default=0,
        choices=[0, 1, 2],
        help="Number of decoder skip connection levels (0=no skip, up to 2)",
    )
