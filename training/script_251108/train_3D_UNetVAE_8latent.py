...
This version uses an 8x8x8 spatial latent map:

  - Encoder produces spatial latent map: mu, logvar shape [B, latent_dim, 8,8,8]
  ...
  - Optional U-Net-style skip connections:
      * up to 3 levels:
          ...
          - skip @ 16^3
          - skip @ 32^3
    (set --no_skip_connections for a pure bottleneck AE/VAE)

KL divergence is computed over the spatial latent.
You can set a very small --kl_beta or 0.0 during reconstruction-focused training.
"""
...
# ----------------------
# Model: 3D U-Net VAE (8x8x8 bottleneck)
# ----------------------
...
class Decoder3DUNetVAE(nn.Module):
    """
    Up: 8 -> 16 -> 32 with configurable skip connections.

    Input:
      z: [B, latent_dim, 8,8,8]
      skips: (e1,e2,e3) from encoder when skip connections are enabled.

    skip_levels:
      0: no skip connections
      1: concat e3 at 8^3 before first upsample
      2: concat e3 at 8^3 and e2 at 16^3
      3: concat e3 at 8^3, e2 at 16^3, and e1 at 32^3
    """
    ...
    def __init__(self, out_ch=3, base=64, latent_dim=256, skip_levels: int = 3):
        ...
        # Process latent at 8^3
        self.rb_latent = ResBlock3D(latent_dim)

        # 8^3 stage + optional e3 skip
        up1_in_channels = latent_dim + (base * 4 if self.skip_levels >= 1 else 0)
        self.up1 = nn.ConvTranspose3d(up1_in_channels, base * 4, 4, stride=2, padding=1)  # 8 -> 16
        self.rb_up1 = ResBlock3D(base * 4)

        # 16^3 stage + optional e2 skip
        up2_in_channels = base * 4 + (base * 2 if self.skip_levels >= 2 else 0)
        self.up2 = nn.ConvTranspose3d(up2_in_channels, base * 2, 4, stride=2, padding=1)  # 16 -> 32
        self.rb_up2 = ResBlock3D(base * 2)

        # 32^3 stage + optional e1 skip
    ...
    def forward(self, z, skips=None):
        if self.use_skip_connections:
            if skips is None:
                # Generation mode: create zero skip features to match channel dimensions
                # This allows generation from prior without encoder features
                B = z.shape[0]
                if self.skip_levels >= 1:
                    # e3 channels = up1.in_channels - latent_dim (from z)
                    e3_channels = self.up1.in_channels - z.shape[1]
                    e3 = torch.zeros(B, e3_channels, 8, 8, 8, device=z.device, dtype=z.dtype)
                else:
                    e3 = None
                if self.skip_levels >= 2:
                    # e2 channels = up2.in_channels - base*4 (from rb_up1 output)
                    base_4_channels = self.rb_up1.net[0].weight.shape[1]  # base*4
                    e2_channels = self.up2.in_channels - base_4_channels
                    e2 = torch.zeros(B, e2_channels, 16, 16, 16, device=z.device, dtype=z.dtype)
                else:
                    e2 = None
                if self.skip_levels >= 3:
                    # e1 channels = out_block.in_channels - base*2 (from rb_up2 output)
                    base_2_channels = self.rb_up2.net[0].weight.shape[1]  # base*2
                    e1_channels = self.out_block[0].in_channels - base_2_channels
                    e1 = torch.zeros(B, e1_channels, 32, 32, 32, device=z.device, dtype=z.dtype)
                else:
                    e1 = None
            else:
                if len(skips) != 3:
                    raise ValueError("Expected 3 skip tensors (e1,e2,e3) from encoder.")
                e1, e2, e3 = skips
        else:
            e1 = e2 = e3 = None
...
class UNet3DVAE(nn.Module):
    ...
    def forward(self, x):
        ...
        z = self.reparameterize(mu, logvar)  # [B, latent_dim, 8,8,8]
...
def kl_divergence(mu, logvar):
    """
    KL(q(z|x) || N(0,I)) over all non-batch dimensions.
    Suitable for spatial latent.
    """
...
def train(args, resume_checkpoint=None):
    ...
    if args.no_skip_connections and args.skip_levels > 0:
        console.print(
            "[yellow]Warning:[/yellow] --no_skip_connections overrides --skip_levels; using 0 skip levels."
        )
    ...
    # Header
    header_text = (
        f"[bold cyan]Minecraft 3D U-Net VAE Training (8x8x8 latent)[/bold cyan]\n"
    ...
    console.print(f"[bold]Skip connections:[/bold] {skip_msg}\n")
    ...
    with Progress(
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            for batch_idx, (onehot, labels) in enumerate(train_loader):
                ...
                with torch.amp.autocast(
                    ...
                    if (
                        epoch == start_epoch
                        and batch_idx == 0
                        and not logits_stats_collected
                    ):
            ...
            progress.console.print(
                f"Epoch {epoch:03d}: train {train_loss:.4f} | val {val_loss:.4f} | {fmt_secs(epoch_secs)}"
                f"{best_marker}{ckpt_marker}"
            )

            # -------- Samples: reconstruction + prior samples --------
            if epoch % args.sample_every == 0:
                ...
                with torch.no_grad():
                    ...
                    # Prior samples: z ~ N(0,I) (true generation from latent space)
                    # Note: Skip connections are NOT used for prior samples
                    # because they are encoder features tied to specific inputs
                    latent_shape = (1, args.latent_dim, 8, 8, 8)
                    for i in range(args.n_samples):
                        # Sample from standard normal distribution
                        z_prior = torch.randn(latent_shape, device=device)
                        # Decode without skip connections for true generation
                        logits_prior = model.decoder(z_prior, skips=None)
    ...
    metadata = {
        ...
        "loss_kl_divergence": "KL(q(z|x) || N(0,I)) (spatial, 8x8x8)",
    }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Minecraft Tree 3D U-Net VAE (8x8x8 latent)")
    ...
    parser.add_argument(
        ...
        default=0,
        choices=[0, 1, 2, 3],
        help="Number of decoder skip connection levels (0=no skip, up to 3)",
    )

    # 建議先用很小的 KL，或 0，專心讓 decoder 學好樹的細節
