...
Key changes from previous ConvVAE:
  - Encoder produces spatial latent map: mu, logvar shape [B, latent_dim, 4, 4, 4]
  - Decoder is 3D U-Net style:
      * 3-level downsampling: 32 -> 16 -> 8 -> 4
      * 3-level upsampling with skip connections (concat) from encoder:
          - skip @ 8^3, 16^3, 32^3
  - KL divergence computed over all latent spatial locations.
  - Samples during training are drawn from q(z|x) using val batch
    (since U-Net decoder使用skip，純 prior N(0,I) 這裡先不用硬塞; 你目前也主看 reconstruction)
...
Output:
  Same as原本腳本: best/last checkpoint, CSV logs, samples (npz+png).
"""
...
def extract_zip_to_temp(zip_path: str, console=None) -> tuple[str, tempfile.TemporaryDirectory]:
    ...
    if not os.path.exists(train_dir):
        ...
        if not found_dirs:
            raise ValueError(
                "Zip file must contain train/val/test subdirectories."
            )
        if len(found_dirs) >= 3:
...
class Decoder3DUNetVAE(nn.Module):
    ...
    def __init__(self, out_ch=3, base=64, latent_dim=256, skip_levels: int = 3):
        ...
        self.use_skip_connections = self.skip_levels > 0  # backwards compatibility

        # 4 -> 8
        self.up1 = nn.ConvTranspose3d(latent_dim, base * 8, 4, stride=2, padding=1)
        self.rb1 = ResBlock3D(base * 8)

        up2_in_channels = base * 8 + (base * 4 if self.skip_levels >= 1 else 0)
        self.up2 = nn.ConvTranspose3d(up2_in_channels, base * 4, 4, stride=2, padding=1)
        self.rb2 = ResBlock3D(base * 4)

        up3_in_channels = base * 4 + (base * 2 if self.skip_levels >= 2 else 0)
        self.up3 = nn.ConvTranspose3d(up3_in_channels, base * 2, 4, stride=2, padding=1)
    ...
    def forward(self, z, skips=None):
        if self.skip_levels > 0:
            if skips is None:
                raise ValueError("Decoder3DUNetVAE requires skip connections (e1,e2,e3) when skip_levels > 0.")
            e1, e2, e3 = skips
        else:
            e1 = e2 = e3 = None

        h = self.up1(z)  # -> [B, base*8, 8,8,8]
        h = self.rb1(h)

        if self.skip_levels >= 1:
            h = torch.cat([h, e3], dim=1)  # cat @ 8^3
        h = self.up2(h)                   # -> [B, base*4, 16,16,16]
        h = self.rb2(h)

        if self.skip_levels >= 2:
            h = torch.cat([h, e2], dim=1)  # cat @ 16^3
        h = self.up3(h)                   # -> [B, base*2, 32,32,32]
        h = self.rb3(h)

        if self.skip_levels >= 3:
            h = torch.cat([h, e1], dim=1)  # cat @ 32^3
        h = self.out_block(h)
...
class UNet3DVAE(nn.Module):
    ...
    def forward(self, x):
        ...
        z = self.reparameterize(mu, logvar)  # [B, latent_dim, 4,4,4]
...
def kl_divergence(mu, logvar):
    """
    KL(q(z|x) || N(0,I))，對所有非 batch 維度取平均。
    適用於有空間 latent 的情況，避免因為維度太多導致數值爆炸。
    """
...
def train(args, resume_checkpoint=None):
    ...
    with Progress(
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            for batch_idx, (onehot, labels) in enumerate(train_loader):
                ...
                with torch.amp.autocast(device_type=device.type, enabled=use_amp):
                    ...
                    if epoch == start_epoch and batch_idx == 0 and not logits_stats_collected:
                        ...
                        console.print(f"[dim]Logits: mean={lf.mean():.4f}, std={lf.std():.4f}, "
                ...
                if epoch == start_epoch and batch_idx == 0:
                    ...
                    console.print(f"Debug: CE={ce_only.item():.4f}, KL={kl_only.item():.4f}, total={loss.item():.4f}")
            ...
            # Samples: reconstruction + posterior samples
    ...
    final_table.add_row("Last Checkpoint", "Deleted after completion" if last_checkpoint_removed else last_checkpoint_path if os.path.exists(last_checkpoint_path) else "None")
    console.print("\n", final_table, "\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--skip_levels', type=int, default=3,
                        choices=[0, 1, 2, 3],
                        help='Number of decoder skip connection levels to enable (0 disables skips)')
