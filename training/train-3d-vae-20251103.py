...
def train(args):
    ...
    # Check if experiment directory exists and is not empty
    ...
    console.print(Panel.fit(
        ...
        f"Started: [green]{train_start_time.strftime('%Y-%m-%d %H:%M:%S')}[/green]",
        border_style="cyan"
    ))
    ...
    # Record training history for CSV
    training_history = []

    # Create progress bar (epochs + 1 for final test)
    with Progress(
        SpinnerColumn(),
        ...
        overall_task = progress.add_task("[cyan]Training", total=args.epochs + 1)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            cum_secs = time.time() - global_t0
            
            # Check if this is the best model
            is_best = val_loss < best_val
            best_marker = " | ★ Best!" if is_best else ""
            progress.console.print(
                f"Epoch {epoch:03d}: train {train_loss:.4f} | val {val_loss:.4f} | {fmt_secs(epoch_secs)}{best_marker}"
            )
            ...
            if is_best:
                best_val = val_loss
                model_path = os.path.join(exp_dir, f'best_{args.exp_name}.pt')
                torch.save({'model': model.state_dict(), 'args': vars(args)}, model_path)
    ...
    final_table.add_row("Model Saved", model_path)
    console.print("\n", final_table, "\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_root', type=str, required=True, help='Directory with train/val/test subdirectories')
    parser.add_argument('--out_dir', type=str, required=True, help='Base output directory')
    parser.add_argument('--exp_name', type=str, required=True, help='Experiment name (creates subdirectory in out_dir)')
