...
著色沿用 ``voxel_label_projections`` 的配色，與三視圖視覺一致：
  - "component"（預設）：wood 以 26-connectivity 上色（最大塊藍、其餘碎塊紅），leaf 綠、air 深。
    可同時看出「密度」與「連通」——浮空碎塊會以警示色出現在它所在的那幾層。
...
# occupancy 模式配色：air 與 component 模式一致，佔據用中性亮色
_OCC_AIR = np.array([0.10, 0.10, 0.12], dtype=np.float32)
...
def save_orthoslice_montage(
    ...
    *,
    ...
    if use_component_legend:
        legend_elements = [
            Patch(facecolor=DISPLAY_COLORS[DISPLAY_LARGEST], edgecolor="none", label="largest wood"),
            Patch(facecolor=DISPLAY_COLORS[DISPLAY_FRAGMENT], edgecolor="none", label="other wood"),
            Patch(facecolor=DISPLAY_COLORS[DISPLAY_LEAF], edgecolor="none", label="leaf"),
            Patch(facecolor=DISPLAY_COLORS[DISPLAY_AIR], edgecolor="none", label="air"),
        ]
        fig.legend(
    ...
    if comp_info is not None:
        ...
        stats_line += f" | wood components: {n_comp} | largest_log_ratio: {llr:.3f}"
    elif color_mode == "component" and not HAS_SCIPY:
        stats_line += " | (scipy missing → occupancy coloring)"
